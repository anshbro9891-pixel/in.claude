...
const STREAM_SYSTEM_PROMPT = `You are INCLAW (Intelligent Neural Cognitive Learning Agent Workflow), an advanced autonomous AI coding agent built in India.
 ...
 *
 * Adds an Ollama streaming fast-path and uses the multi-backend router for non-streaming requests.
...
export async function POST(request: NextRequest) {
  ...
  let modelId = "qwen2.5-coder-32b";
  let modelName = "INCLAW";
  let ollamaTag = "qwen2.5-coder:32b";
  ...
  try {
    ...
    modelId = modelInfo?.id ?? model ?? modelId;
    modelName = modelInfo?.name ?? model ?? modelName;
    ollamaTag = modelInfo?.ollamaTag ?? model ?? ollamaTag;
    ...
    const result = await routeLLM(llmMessages, modelId);
    ...
    if (messages && messages.length > 0) {
      const fallbackContent = generateAgenticResponse(messages, modelId, modelName);
